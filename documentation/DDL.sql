...
--
-- Host: localhost
-- Generation Time: Mar 13, 2026 at 04:45 PM
...
CREATE DEFINER=`cs340_parknic`@`%` PROCEDURE `sp_delete_brew_recipe` (IN `p_recipe_id` INT)   BEGIN
...
CREATE DEFINER=`cs340_parknic`@`%` PROCEDURE `sp_delete_coffee` (IN `p_coffee_id` INT)   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        SELECT 'Delete error!' AS result;
        ROLLBACK;
...
CREATE DEFINER=`cs340_parknic`@`%` PROCEDURE `sp_delete_many_brew_results` (IN `p_recipe_id` INT, IN `p_coffee_id` INT)   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
...
CREATE DEFINER=`cs340_parknic`@`%` PROCEDURE `sp_delete_one_brew_result` (IN `p_result_id` INT)   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        SELECT 'Delete error!' AS result;
        ROLLBACK;
...
CREATE DEFINER=`cs340_parknic`@`%` PROCEDURE `sp_insert_brew_result` (IN `recipe_id` INT, IN `coffee_id` INT, IN `actual_dose` DECIMAL(5,2), IN `actual_yield` DECIMAL(6,2), IN `actual_grind_size` DECIMAL(4,2), IN `actual_water_temp` DECIMAL(5,2), IN `actual_brew_time` TIME, IN `tds_reading` DECIMAL(3,2), IN `ext_yield` DECIMAL(4,2), IN `rating` INT, OUT `result_id` INT)   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
...
CREATE DEFINER=`cs340_parknic`@`%` PROCEDURE `sp_insert_coffee` (IN `p_coffee_name` VARCHAR(50), IN `p_roaster_id` INT, IN `p_roast_type_id` INT, IN `p_lot_id` INT)   BEGIN
...
CREATE DEFINER=`cs340_parknic`@`%` PROCEDURE `sp_reset_db` ()   BEGIN
    SET FOREIGN_KEY_CHECKS = 0;
...
CREATE DEFINER=`cs340_parknic`@`%` PROCEDURE `sp_update_brew_recipe` (IN `p_recipe_id` INT, IN `p_brewer_id` INT, IN `p_target_dose` DECIMAL(5,2), IN `p_target_yield` DECIMAL(6,2), IN `p_target_grind_size` DECIMAL(4,2), IN `p_target_water_temp` DECIMAL(5,2), IN `p_target_brew_time` TIME, IN `p_status_id` INT)   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
...
DROP PROCEDURE IF EXISTS `sp_update_coffee_lot_varietals` $$

CREATE DEFINER=`cs340_parknic`@`%` PROCEDURE `sp_update_coffee_lot_varietals` (IN `p_old_lot_id` INT, IN `p_old_varietal_id` INT, IN `p_new_lot_id` INT, IN `p_new_varietal_id` INT)   BEGIN
...
INSERT INTO
    `BrewerTypes` (`brewer_id`, `brewer_type`)
VALUES (1, 'Fetco'),
    (2, 'April'),
    (3, 'Kalita Wave'),
    (4, 'Chemex'),
...
INSERT INTO
    `CoffeeLotVarietals` (`lot_id`, `varietal_id`)
VALUES (1, 2),
    (1, 3),
    (1, 4),
    (2, 5),
...
INSERT INTO
    `ProcessingStyles` (`process_id`, `process_name`)
VALUES (1, 'Washed'),
    (2, 'Anaerobic'),
    (3, 'Natural'),
    (4, 'Honey');
...
INSERT INTO
    `RecipeStatuses` (`status_id`, `status_type`)
VALUES (1, 'Active'),
    (2, 'Testing'),
    (3, 'Retired');
...
VALUES (
        1,
        'Heart',
        'wholesale@heartroasters.com',
        1
    ),
    (
        2,
        'Prodigal',
        'wholesale@prodigalcoffee.com',
        2
    ),
    (
        3,
        'Coffee Collective',
        'coffee@coffeecollective.dk',
        3
    ),
    (
        4,
        'Prolog',
        'wholesale@prologcoffee.com',
        3
    );
...
INSERT INTO
    `RoastTypes` (`roast_type_id`, `roast_name`)
VALUES (1, 'Light'),
    (2, 'Medium'),
    (3, 'Espresso'),
    (4, 'Dark');
...
--
ALTER TABLE `BrewerTypes` ADD PRIMARY KEY (`brewer_id`);
...
--
ALTER TABLE `ProcessingStyles` ADD PRIMARY KEY (`process_id`);

--
-- Indexes for table `RecipeStatuses`
--
ALTER TABLE `RecipeStatuses` ADD PRIMARY KEY (`status_id`);
...
--
ALTER TABLE `RoastTypes` ADD PRIMARY KEY (`roast_type_id`);
...
MODIFY `brewer_id` int(11) NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 6;
...
--
ALTER TABLE `Roasters`
MODIFY `roaster_id` int(11) NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 5;
...
MODIFY `roast_type_id` int(11) NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 5;
...
--
ALTER TABLE `Varietals`
MODIFY `varietal_id` int(11) NOT NULL AUTO_INCREMENT,
AUTO_INCREMENT = 10;
