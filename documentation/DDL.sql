...
--
-- Host: localhost
-- Generation Time: Mar 16, 2026 at 12:55 PM
...
CREATE PROCEDURE `sp_add_coffeelot` (IN `p_lot_number` VARCHAR(50), IN `p_location_id` INT, IN `p_meters_elevation` INT, IN `p_process_id` INT)   BEGIN
...
CREATE PROCEDURE `sp_add_coffee_lot_varietal` (IN `p_lot_id` INT, IN `p_varietal_id` INT)   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        SELECT 'Insert error!' AS result;
        ROLLBACK;
...
CREATE PROCEDURE `sp_add_varietal` (IN `p_varietal_name` VARCHAR(50))   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        SELECT 'Insert error!' AS result;
        ROLLBACK;
...
CREATE PROCEDURE `sp_delete_coffeelot` (IN `p_lot_id` INT)   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        SELECT 'Delete error!' AS result;
        ROLLBACK;
...
CREATE PROCEDURE `sp_delete_coffeelotvarietal` (IN `p_lot_id` INT, `p_varietal_id` INT)   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        SELECT 'Delete error!' AS result;
        ROLLBACK;
...
CREATE PROCEDURE `sp_delete_varietal` (IN `p_varietal_id` INT)   BEGIN
    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        SELECT 'Delete error!' AS result;
        ROLLBACK;
...
CREATE PROCEDURE `sp_update_coffee_lot_varietal` (IN `p_old_lot_id` INT, IN `p_old_varietal_id` INT, IN `p_new_lot_id` INT, IN `p_new_varietal_id` INT)   BEGIN
